...
meta %>% group_by(`Birth Year (HOBBS)`, Phenotype) %>% mutate(Count=n()) %>% dplyr::select(`Birth Year (HOBBS)`, Phenotype, Count) %>% unique() %>% filter(`Birth Year (HOBBS)` == 2012)
```

```{r}
plot2012<-meta %>% filter(`Birth Year (HOBBS)` ==2012) %>% group_by(lat,lon,Phenotype) %>% mutate(Count=n()) %>% dplyr::select(lat,lon,Phenotype,Count) %>% unique()
...
## Make a new bamlist
```{r}
meta2012<-meta %>% filter(`Birth Year (HOBBS)` ==2012)
