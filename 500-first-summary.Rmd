...
```
What I find particularly interesting about this is that there are a lot of values around 0.50 and 0.75.    
...
I conducted two different genome wide association studies on the data (angsd and plink). These methods are largely based around using a chi-squared test, and, often a p-value of 5 x 10-8 is considered meaningful. Neither test did find significant results in the conventional sense, which often seems to occur. Here are the top results from the ANGSD association test, and a Manhattan style plot of Chromosome 58.   
...
ggplot(data%>%filter(Chromosome %in% c(58) & log10p>=1))+
  geom_point(aes(x=Position, y=log10p, color=log10p), alpha=0.75, cex=0.7)+
...
Considering the top 1% of results shared between separate tests has been reported in the literature as a method of determine which SNPs may be associated with the trait. This approach highlights 43 SNPs for me, mostly found on the assembly contigs 114514 and 58. This includes top SNPs shown by DAPC to be able to separate the two phenotypes genetically.
